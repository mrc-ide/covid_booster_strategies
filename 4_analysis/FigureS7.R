...
rq3_doses <- ggplot(data = filter(df1,strategy_name != "Pre-vaccine introduction"), aes(x = as.Date(date), y = vaccines_t/target_pop, col = strategy_name)) +
...
rq3_hosp <- ggplot(data = df1, aes(x = as.Date(date), y = hosp_t/target_pop * 1e6, col = strategy_name)) +
...
rq3_inc <- ggplot(data = df1, aes(x = as.Date(date), y = inc_t/target_pop * 1e6, col = strategy_name)) +
...
rq3_summary_plot <- ggplot(data = df_summarydat, aes(x = strategy_name, y = value / target_pop * 1e6, fill = strategy_name)) +
...
rq3_additional <- ggplot(data = df_add1, aes(x = strategy_name, y = value, fill = strategy_name)) +
...
rq3_nat <- ggplot(data = df1_nat, aes(x = as.Date(date), y = titre, col = nat_label, linetype = nat_label)) +
...
combined
ggsave("plots/FigureS7.png", combined, height = 10, width = 11)
