...
scenarios <- expand_grid(income_group = income_group,
  ...
  mutate(ICU_scal_vfr = if_else(hosp_scal_vfr == 1, 1, ICU_scal_vfr),
         hosp_scal_vfr = if_else(ICU_scal_vfr == 1, 1, hosp_scal_vfr),
         ICU_scal_vfr2 = if_else(hosp_scal_vfr2 == 1, 1, ICU_scal_vfr2),
         hosp_scal_vfr2 = if_else(ICU_scal_vfr2 == 1, 1, hosp_scal_vfr2)) %>%
  filter(!(vfr2 == 1 & hosp_scal_vfr2 == 1)) %>%
  # mutate(omicron_vaccine = if_else(vfr2 != vfr, 0, omicron_vaccine),
  #        omicron_vaccine = if_else(hosp_scal_vfr2 == 1, 0, omicron_vaccine),
  #        omicron_vaccine = if_else(vaccine_doses == 3, 0, omicron_vaccine)) %>%
  # don't apply 10% transmission increase to no-variant scenario
  mutate(max_Rt_var2_scal = if_else((vfr2 == vfr & hosp_scal_vfr2 == hosp_scal_vfr), 1, max_Rt_var2_scal)) %>%
  unique()
...
## Run
runs <- run$enqueue_bulk(scenarios[c(301:311,369:373,423:429),], run_scenario, do_call = TRUE, progress = TRUE)
runs$status()
