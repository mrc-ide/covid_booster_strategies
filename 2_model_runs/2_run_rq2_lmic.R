...
fit <- "UKHSA_v6_65+_20220702_AZPD2=FALSE_SB=FALSE_NewDecay=TRUE_AddBst=FALSE_AltSev=FALSE"
...
vacc_per_week <- 0.02
t_d3 <- 365 
strategy <- c("same_doses", "two_doses_only")
...
scenarios <- expand_grid(fit = fit,
  ...
  mutate(age_groups_covered_d3 = age_groups_covered,
         age_groups_covered_d4 = age_groups_covered_d3,
  ...
  filter((strategy == "same_doses" & vaccine_doses %in% c(2,3)) | (strategy == "two_doses_only" & vaccine_doses == 2)) %>%
  unique()
