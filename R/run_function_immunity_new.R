...
run_scenario_new <- 
  function(name = "scenario1",
    ...
    attach_event_listeners_independent_nat(variables = variables, events = events, parameters = parameters, dt = dt, additive = FALSE)
    ...
    sp_render_process_daily <- function(renderer, variable1, variable2, dt) {
      ...
      function(t) {
        if ((t * dt) %% 1 == 0) {
          day <- as.integer(t * dt)
          n1 <- exp(variable1$get_values())
          n2 <- exp(variable2$get_values())
          nat <- pmax(n1,n2)
